...
class Board(object):
    ...
    def neighbors(self, territory_id):
        """
        Create a generator of
        all territories neighboring a given territory.
    ...
    def friendly_neighbors(self, territory_id):
        ...
        territories neighboring a given territory, of which
        the owner is the same as
        the owner of the original territory.
        ...
        Returns:
            ...
            of the form (territory_id, player_id, armies).
        """
        player_id = self.owner(territory_id)
        neighbor_ids = risk.definitions.territory_neighbors[territory_id]
        return (t for t in self.data if
    ...
    def is_valid_path(self, path):
        ...
        next territory W is in the neighbors if V.
    ...
    def is_valid_attack_path(self, path):
        ...
        All valid attacks, therefore, will
        follow a path of starting on one player's
    ...
    def cost_of_attack_path(self, path):
        '''
        The cost of an attack path is the
        total number of enemy armies in the path.
        In other words, it is the total number of
        armies in the subpath starting at the second vertex.

        Args:
            path ([int]): a list of
            territory_ids which must be a valid attack path
    ...
    def can_fortify(self, source, target):
        ...
        and target territories that is owned entirely by the same player.
        ...
        Returns:
            bool: True if reinforcing
            the target from the source territory is a valid move
        '''
        dictionary = {}
        dictionary[source] = [source]
        queue = deque()
    ...
    def cheapest_attack_path(self, source, target):
        ...
        or there may be no valid
        attack paths (in which case the function returns None).
    ...
    def reinforcements(self, player_id):
        ...
        Returns:
            int: Number of reinforcement
            armies that the player is entitled to.
        """
    ...
    def fortify(self, from_territory, to_territory, n_armies):
        ...
        Raises:
            ValueError if the
            player moves too many or negative armies.
            ValueError if the territories do
            not share a border or are not owned by the same player.
        """
    ...
    def attack(self, from_territory, to_territory, attackers):
        ...
        Args:
            from_territory (int): Territory_id
            of the offensive territory.
            to_territory (int): Territory_id of
            the defensive territory.
            attackers (int): Number of attacking armies.

        Raises:
            ValueError if the number of
            armies is <1 or too large.
    ...
    def plot_board(self, path=None, plot_graph=False, filename=None):
        ...
        Args:
            ...
            plot_graph (bool): if true, plots the
            graph structure overlayed on the board
            filename (str): if given, the plot will
            be saved to the given filename instead of displayed
        """
    ...
    def fight(cls, attackers, defenders):
        ...
        Returns:
            tuple (int, int): Number of lost
            attackers, number of lost defenders.
        """
    ...
    def mobile(self, player_id):
        """
        Create a generator of all territories of
        a player which can attack or move,
